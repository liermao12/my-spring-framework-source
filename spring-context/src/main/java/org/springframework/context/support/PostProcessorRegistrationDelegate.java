...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			for (BeanFactoryPostProcessor postProcessor : beanFactoryPostProcessors) {
				...
				if (postProcessor instanceof BeanDefinitionRegistryPostProcessor) {
					...
					// 这里调用 可以向bf 内部再次注册一些 bd信息。
					registryProcessor.postProcessBeanDefinitionRegistry(registry);
					// 添加到集合。
					registryProcessors.add(registryProcessor);
				}
				else {
					//因为当前postProcessor 非 registry类型，即当前postProcessor是个普通的。就加入到普通集合，后续会统一执行一些方法。
			...
			for (String ppName : postProcessorNames) {
				...
				if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) {
					...
					// 因为接下来马上就是要执行 bdrpp 的 registry 相关接口方法，所以将beanName添加到processedBeans 表示已执行。
					processedBeans.add(ppName);
				}
			}
			//排序，根据order值进行 升序排序。
			sortPostProcessors(currentRegistryProcessors, beanFactory);
			//添加到 registry类型后处理器集合内。
			registryProcessors.addAll(currentRegistryProcessors);
			// 调用每个bdrpp 的postProcessBeanDefinitionRegistry 方法，向bf内注册bd信息。
			...
			// 接口方法执行之后 会向bf 内注册bd ，再次注册的 bd 也有可能是 bdrpp 类型，所以，需要该变量控制 while循环。
			...
			// 因为registry类型的后处理器 继承自 BeanFactoryPostProcessor ，bfpp 类型后处理器有 postProcessBeanFactory() 接口方法需要执行
			// 所以 ， registry 类型的所有后处理器 不仅要执行 postProcessBeanDefinitionRegistry() 还要执行 postProcessBeanFactory()
			invokeBeanFactoryPostProcessors(registryProcessors, beanFactory);
			// 执行硬编码提供的 beanFactoryPostProcessor接口实现类的 postProcessBeanFactory() 方法。
		...
		//上面的代码处理了： 硬编码提供的bfpp 和 bdrpp 这两种类型的 bfpp.
		//还有普通的bfpp需要处理。
	...
	public static void registerBeanPostProcessors(
		...
		// 首先注册实现了主排序接口的 后处理器
		sortPostProcessors(priorityOrderedPostProcessors, beanFactory);
		// 注册。内部bf.addBeanPostProcessor 确保了不会重复添加 后处理器。
	...
	private static final class BeanPostProcessorChecker implements BeanPostProcessor {
		...
		public Object postProcessAfterInitialization(Object bean, String beanName) {
			//条件一：成立 说明 当前bean类型是普通bean ，不是后处理器类型。
			//条件二：成立 说明 当前beanName是用户级别的bean ，非spring框架自身的bean.
			//条件三：成立 说明 bf上面注册的后处理器数量 < 后处理器总数，说明当前bean创建阶段时，后处理器框架尚未初始化完毕..需要打印日志提示
